...
def transform_counterparty(df_dict):
    ...
    dim_counterparty_df["counterparty_legal_address_line_1"] = df_dict["address_df"][
        ["address_line_1"]
    ]
    dim_counterparty_df["counterparty_legal_address_line_2"] = df_dict["address_df"][
        ["address_line_2"]
    ]
    dim_counterparty_df["counterparty_legal_district"] = df_dict["address_df"][
        ["district"]
    ]
    dim_counterparty_df["counterparty_legal_city"] = df_dict["address_df"][["city"]]
    dim_counterparty_df["counterparty_legal_postal_code"] = df_dict["address_df"][
        ["postal_code"]
    ]
    dim_counterparty_df["counterparty_legal_country"] = df_dict["address_df"][
        ["country"]
    ]
    dim_counterparty_df["counterparty_legal_phone_number"] = df_dict["address_df"][
        ["phone"]
    ]
...
def transform_currency(df_dict):
    ...
    dim_currency_df["currency_name"] = df_dict["currency_df"]["currency_code"].replace(
...
def transform_location(df_dict):
    ...
    dim_location_df["location_id"] = df_dict["sales_order_df"][
...
def setup():
    ...
    table_keys = [
        elem["Key"] for elem in s3.list_objects(Bucket=bucket_name)["Contents"]
    ]
    df_dict = {}
